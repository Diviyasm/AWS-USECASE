...
resource "aws_launch_template" "my_public_instance" {
  name          = "my-public-instance-template"
  instance_type = "t2.micro"
  image_id      = "ami-055e3d4f0bbeb5878" 
  iam_instance_profile {
    name = aws_iam_instance_profile.my_public_role.name
  }
  vpc_security_group_ids = [aws_security_group.my_public_sg.id]
}

# Auto Scaling Group for Public EC2 Instances
resource "aws_autoscaling_group" "my_public_asg" {
  desired_capacity    = 2
  max_size            = 3
  min_size            = 1
  vpc_zone_identifier = [
  ...
  launch_template {
    id      = aws_launch_template.my_public_instance.id
    version = "$Latest"
  }
  target_group_arns = [aws_lb_target_group.my_app_targets.arn]
}
...
resource "aws_lb_target_group" "my_app_targets" {
  name     = "my-app-targets"
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
...
resource "aws_lb_listener" "my_app_listener" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.my_app_targets.arn
  }
}
...
resource "aws_lb_target_group" "my_network_targets" {
  name     = "my-net-targets"
  port     = 80
  protocol = "TCP"
  vpc_id   = aws_vpc.main.id
...
resource "aws_lb_listener" "my_net_listener" {
  load_balancer_arn = aws_lb.my_network_lb.arn
  port              = 80
  protocol          = "TCP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.my_network_targets.arn
  }
}
...
resource "aws_s3_bucket_ownership_controls" "my_private_bucket_ownership_controls" {
  bucket = aws_s3_bucket.my_private_bucket.id # Associates the ownership controls with the bucket
  rule {
...
resource "aws_s3_bucket_acl" "my_private_bucket_acl" {
  depends_on = [aws_s3_bucket_ownership_controls.my_private_bucket_ownership_controls] # Ensures ownership controls are created first
  bucket = aws_s3_bucket.my_private_bucket.id # Associates the ACL with the bucket
  acl = "private" # Applies private ACL
}

# S3 Bucket Versioning
resource "aws_s3_bucket_versioning" "my_s3_versioning" {
  bucket = aws_s3_bucket.my_private_bucket.id # Associates versioning with the bucket
...
# IAM Role
resource "aws_iam_role" "my_public_role" {
  name               = "my-public-role"
...
resource "aws_iam_policy" "my_s3_access" {
  name        = "my-s3-access-unique"
    ...
    Statement = [
      {
        Action   = ["s3:*"],
        Effect   = "Allow",
        Resource = [aws_s3_bucket.my_private_bucket.arn, "${aws_s3_bucket.my_private_bucket.arn}/*"]
      }
    ]
  })
}
...
resource "aws_iam_role_policy_attachment" "my_attach_policy" {
  role       = aws_iam_role.my_public_role.name
  policy_arn = aws_iam_policy.my_s3_access.arn
}

# IAM Instance Profile
resource "aws_iam_instance_profile" "my_public_role" {
  name = "my-public-instance-profile-unique"
  role = aws_iam_role.my_public_role.name
}
